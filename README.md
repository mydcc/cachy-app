...
- **Linting:**
  ...
  >
  > Warnings are capped by a **ratchet**: CI runs `eslint . --max-warnings 173`, the size of the pre-existing `no-explicit-any` / `no-unused-vars` backlog. That number may only ever be *lowered*, never raised, so the backlog can shrink but cannot grow. When you fix warnings, lower the ceiling in `.github/workflows/audit.yml` to match. See `docs/ROADMAP.md` item 21.
  >
