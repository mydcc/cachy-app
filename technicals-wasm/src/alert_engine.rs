...
pub enum AlertCondition {
    ...
    PriceCrossDown(f64),
    PriceReached(f64),
    PriceMovedPct(f64), // Used to trigger after percentage move, if needed later
}
...
impl AlertEngine {
    ...
    pub fn evaluate(&mut self, symbol: &str, current_price: f64, timestamp: i64) -> Vec<AlertEvent> {
            ...
            match alert.condition {
                AlertCondition::PriceReached(target) => {
                    if let Some(last_price) = last_price_opt {
                        ...
                        // Edge case: evaluate against gap in initial data
                        if (current_price - target).abs() < f64::EPSILON {
                            triggered = true;
                        }
                    }
                },
                ...
                AlertCondition::PriceMovedPct(_) => {
                    // Not strictly defined in FEAT-0027 as standard criteria yet, placeholder
                }
            }
