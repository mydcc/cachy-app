...
| File | Symbols |
|------|---------|
| `src/actions/tracking.ts` | handleClick, event |
...
| `src/components/inputs/GeneralInputs.svelte` | setTradeType, GeneralInputs, type |
| `src/components/inputs/TakeProfitTargets.svelte` | removeRow, addRow, index, TakeProfitTargets |
| `src/components/inputs/TradeSetupInputs.svelte` | y, event, event, handleFetchPriceClick, e, ... |
| `src/components/layout/Header.svelte` | Header |
| `src/components/results/PlaceOrderPanel.svelte` | currentSym, detailText, detail, norm, livePrice, ... |
| `src/components/results/SummaryResults.component.test.ts` | writeText |
| `src/components/results/SummaryResults.svelte` | handleToggleLock, handleCopy, SummaryResults |
| `src/components/settings/AccountCard.svelte` | AccountCard |
| `src/components/settings/AccountList.svelte` | confirmSwitch, AccountList, target, confirmedAt |
...
| `src/components/shared/FlashCard.svelte` | handleUnknown, handleKnown, FlashCard, handleFlip |
| `src/components/shared/FloatingIframeButton.svelte` | e, FloatingIframeButton, ch, handleContextMenu, openChannel |
| `src/components/shared/FundingRatePopover.svelte` | FundingRatePopover, closePopover, togglePopover, openPopover |
| `src/components/shared/GlobalTracker.svelte` | name, target, maxDepth, firstClass, stack, ... |
| `src/components/shared/IndicatorPaneHeader.svelte` | IndicatorPaneHeader |
| `src/components/shared/JournalContent.svelte` | JournalContent, file, SortField, preset, applyColumnPreset, ... |
| `src/components/shared/LeftControlPanel.svelte` | LeftControlPanel |
| `src/components/shared/LeverageModal.svelte` | next, confirm, nudge, by, LeverageModal, ... |
| `src/components/shared/MarginModeModal.component.test.ts` | onconfirm |
| `src/components/shared/MarginModeModal.svelte` | MarginModeModal, confirm |
| `src/components/shared/MarketDashboardModal.svelte` | signalOf, price, scoreTooltip, tone, formatPrice, ... |
| `src/components/shared/MarketOverview.svelte` | val, formatValue, MarketOverview, loadToCalculator, decimals |
| `src/components/shared/ModalFrame.svelte` | ModalFrame |
| `src/components/shared/OfflineBanner.svelte` | handleSettings, OfflineBanner |
| `src/components/shared/OnboardingSpotlight.svelte` | OnboardingSpotlight |
| `src/components/shared/OpenOrdersList.svelte` | day, minutes, handleCancel, month, type, ... |
| `src/components/shared/OrderDetailsTooltip.svelte` | rawType, order, formatDate, getOrderType, Props, ... |
| `src/components/shared/OrderHistoryList.svelte` | getTypeLabel, date, handleKeyDown, event, handleLoadMore, ... |
| `src/components/shared/PositionsList.svelte` | roi, getPnlDisplay, val, togglePnlMode, nextMode, ... |
| `src/components/shared/PositionsSidebar.svelte` | event, handleAdjustMarginSuccess, handleTpSlCreateSuccess, closeContextMenu, mode, ... |
| `src/components/shared/PowerToggle.svelte` | handleToggle, PowerToggle, input, event |
| `src/components/shared/QuizButton.svelte` | QuizButton |
| `src/components/shared/RangeSlider.svelte` | RangeSlider |
| `src/components/shared/TakeProfitRow.svelte` | val, target, currentTargets, handlePercentInput, newPercent, ... |
| `src/components/shared/TakeProfitTargets.svelte` | removeRow, index, TakeProfitTargets, addRow |
| `src/components/shared/Tooltip.svelte` | Tooltip |
| `src/components/shared/TpSlCreateModal.svelte` | TpSlCreateModal, order, openEdit, handleEditSuccess |
| `src/components/shared/TpSlEditModal.svelte` | TpSlEditModal |
| `src/components/shared/TpSlList.svelte` | order, o, getTypeLabel, formatDate, d, ... |
| `src/components/shared/TpSlPriceInput.svelte` | TpSlPriceInput, commit, event, onFieldKey |
...
| `src/components/shared/journal/JournalDeepDive.svelte` | JournalDeepDive, handleCalendarClick, detail |
| `src/components/shared/journal/JournalStatistics.svelte` | JournalStatistics |
| `src/components/shared/journal/JournalTable.svelte` | formatDuration, entry, riskDec, winW, sl, ... |
| `src/components/shared/journal/TradeDetailDrawer.svelte` | input, TradeDetailDrawer, e, handleFileInputChange, e, ... |
| `src/lib/components/ContentRenderer.svelte` | ContentRenderer |
| `src/lib/marketDashboard.ts` | analysis, trendCellClass, state, signalFor |
| `src/lib/windows/implementations/ChannelView.svelte` | handleEmbedError, openInNewTab, ChannelView, retryEmbed |
| `src/lib/windows/implementations/ChatTestView.svelte` | ChatTestView |
| `src/locales/i18n.ts` | _ |
| `src/routes/+layout.server.ts` | load |
| `src/routes/+page.svelte` | handleThemeSwitch, handlePresetLoad, currentIndex, direction, handleTiltAnimationEnd, ... |
...
| `src/services/calculatorService.ts` | storedRisk, calculatedTpDetails, currentTradeState, storedPercent, riskAmount, ... |
| `src/services/exchange/subscriptionLedger.test.ts` | symbol, t, channel |
| `src/services/exchangeCapabilities.ts` | unsupportedTimeInForceReasonKey, exchange |
| `src/services/trackingService.ts` | id, eventData, category, action, type, ... |
| `src/stores/analysis.svelte.ts` | TrendState, SymbolAnalysis, AnalysisQuality |
...
| `src/utils/utils.ts` | formatDynamicDecimal, formatted, dec, maxPlaces, value |
