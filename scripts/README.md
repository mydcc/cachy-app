...
| `deploy-build.yml` | `.github/workflows/deploy-build.yml` (push to `develop`/`main`) | Builds the production artifact in GitHub Actions and publishes it as `cachy-build.tar.gz` on a per-branch moving release tag (`deploy-beta` for `develop`, `deploy-stable` for `main`). `deploy.sh --ci` downloads that artifact instead of compiling on the server — the fix for OOM-killed builds on small hosts. |
