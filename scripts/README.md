...
| `deploy-build.yml` | `.github/workflows/deploy-build.yml` (push to `develop`/`main`) | Builds the production artifact in GitHub Actions and publishes it as `cachy-build.tar.gz` on a per-branch moving release tag (`deploy-beta` for `develop`, `deploy-stable` for `main`). Preserves the previous artifact as `<tag>-previous` for rollback. `deploy.sh --ci` downloads that artifact instead of compiling on the server — the fix for OOM-killed builds on small hosts. |
| `backlog-index.mjs` | `.github/workflows/audit.yml`, `npm run backlog:index` / `backlog:check` | Validates every `docs/backlog/` item's front matter (`--check`, no dependencies) and regenerates `INDEX.md` + `backlog.generated.*`. No bots: the fix PR commits the regenerated files itself, and `audit.yml` fails backlog-touching PRs with a stale index. |
| `check-backlog-flip.ts` (+ `lib/backlog-flip.ts`) | `.github/workflows/pr-body-lint.yml` | Fails a PR whose `Fixes #N` points at a backlog mirror issue without flipping that item to `done` in its own diff — the pre-merge replacement for the deleted auto-done bot. Pure verdict logic in the lib, unit-tested; API flakes fail open, findings fail closed. |
