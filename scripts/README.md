...
| `backlog-auto-done.mjs` | `.github/workflows/backlog-auto-done.yml` (PR merged into `develop` with a `Fixes #N` trailer) | Flips the linked backlog item to `done` on `bot/backlog-auto-done` and regenerates `INDEX.md` + `backlog.generated.*` into the same commit, so one merge produces exactly one bot PR (opened directly, with auto-merge). Pure routing helper `lib/bot-pr-fold.ts` (`fold-into-auto-done` / `update-index-pr` / `create-index-pr`) lets the sync workflows fold into that branch instead of opening a second PR. |
