...
if cargo build --release --target wasm32-unknown-unknown; then
    ...
    echo "❌ WASM build failed (cargo build error)."
    cd ..
    if [[ -f "static/wasm/technicals_wasm.wasm" ]]; then
        echo "⚠️  Falling back to pre-compiled WASM binary in static/wasm/."
        exit 0
    else
        echo "🔴 Critical: No pre-compiled WASM binary found. Build cannot proceed."
        exit 1
    fi
fi
