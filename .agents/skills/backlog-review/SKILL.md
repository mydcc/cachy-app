---
name: backlog-review
description: Review open PRs against backlog acceptance criteria and Cachy rules, reconcile with bot review, confirm fixes in chat, fix findings, commit, push, and re-trigger bot review
---

Review open PRs in `mydcc/cachy-app` against their backlog item (if linked) and Cachy's non-negotiable rules (CLAUDE.md, AGENTS.md). This is a chat-first pipeline — review, reconcile, confirm in chat, then fix: list all findings first, reconcile them with the bot review, present each finding with a fix proposal in chat and wait for confirmation, fix only confirmed findings, commit, push, and comment only after the fixes are pushed — never merge. A PR comment without preceding fixes must never happen: a comment is useless if nothing was fixed. There is no report-only mode in interactive sessions: a run is complete only when every confirmed finding is fixed and pushed, the clean diff is noted in chat, or the run aborted with its blocker stated.
...
- **Artifacts stay English.** PR review comments (step 12), commit messages, code, identifiers, and technical terms remain in English per the repo's Commits & Branches rule. Never translate code identifiers or technical terms into German — only the conversation around them is German.
- **Chat review ends with a plain findings list.** The step 10 chat review lists one numbered bullet per finding, each tagged with its severity (e.g. `1. [MEDIUM] …`) plus a one-line fix proposal. No tables. What was fixed and pushed is documented only in the PR comment (step 12, "Fixed & Pushed" section), so the next review sees it there — never in chat.
...
1. **OCR delegation pre-filter (best-effort, read-only, at most one call).**
    ...
    - Treat the output as input only: the reviewable-file list scopes steps 2–11. Cachy rules (step 5) always win on conflict.
...
2. **Identify the backlog item.**
   - **Already reviewed?** Search the PR comments for a `Code Review for <sha>` marker matching the current HEAD SHA (`gh pr view <nr> --comments --jq '.[].body'`). On a match, end the run immediately with "already reviewed at <sha>" — no new review, no comment. Steps 9 and 12 reuse this lookup for the bot comment.
...
9. **Reconcile with the bot review.**
   ...
   - The reconciled list is the confirmation backlog for step 10 — nothing else gets proposed for fixing.

10. **Chat review and per-finding confirmation (always — no comment before this).**
    - Present the reconciled findings in chat (German): one numbered bullet per finding, each with severity tag and a one-line fix proposal (e.g. `1. [MEDIUM] Missing $effect cleanup in … — Fix: return unsubscribe in …`).
    - Then wait for the user's confirmation. Default is all findings; the user vetoes by number (fine-grained, e.g. "fix 1 and 3, skip 2"). There is no timeout and no self-fixing on silence — without an answer, wait indefinitely; never fix unconfirmed findings on your own.
    - Clean diff (no findings): post no PR comment — it would be noise. End with one short chat line (e.g. "Clean, nichts zu tun.") and finish the run.
    - Findings that cannot be fixed (CI gate, branch conflict, missing access) go back to chat with the blocker stated — never into a PR comment.
    - In CI/unattended runs (bash denied, e.g. the `opencode.yml` review job) there is no chat to confirm in: skip this step and stay report-only (see step 12). This is the only report-only exception.

11. **Apply confirmed fixes (only confirmed findings).**
    ...
    - Fix every finding the user confirmed in step 10, in severity order `CRITICAL` → `HIGH` → `MEDIUM` → `LOW`. Unconfirmed findings are left untouched, no exclusions on the confirmed set: sensitive code paths (`area: execution`, `area: security`, `area: exchange`, `priority: P0`) are fixed like everything else; the step 7 gentle human-review note stays as an info line but never blocks a fix.
...
12. **Post a Comment and re-trigger the bot review (only after fixes are pushed).**
   ...
   - For interactive sessions (CLI, local pairing) where no platform wrapper automatically publishes output, use `add_issue_comment` or post the comment with this structure:
    ...
    - Only when step 11 pushed fixes: after the review comment, post a **separate** comment whose body starts with `/review`. The workflow trigger matches `startsWith('/review')`, so it must be its own comment — this asks the bot for a fresh review of the fixed state.
...
- **Writes are scoped to step 11.** Code edits, commits, and pushes only there, only on the PR head branch. Never merge, never touch `develop`/`main`, never edit backlog files. The PR comments (step 12) are the only other writes, and they never precede the fixes.
