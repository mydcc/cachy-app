...
- **Land PRs via squash-merge.** Release notes are generated from main's history with one entry per commit — an unsquashed PR lands every fixup push as its own duplicate entry (see the 5× `alerts: implement local alert engine` pile-up cleaned in 2026-09).
