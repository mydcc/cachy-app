...
<script lang="ts">
  ...
  async function handleSave() {
    if (!triggerPrice) {
      error = $_("dashboard.tpslManager.triggerRequired");
      return;
    }
    ...
    try {
      ...
      const msg = e.message;
      if (msg && (msg.startsWith("apiErrors.") || msg.startsWith("bitunixErrors."))) {
          // @ts-ignore - dynamic key
          error = $_(msg);
      } else {
          error = msg || $_("dashboard.tpslManager.failedToModify");
      }
    } finally {
      loading = false;
    }
...
<ModalFrame
  title={$_("dashboard.tpslManager.editTitle", {
    values: { type: order?.planType === 'PROFIT' ? 'Take Profit' : 'Stop Loss' }
  })}
  {onclose}
  isOpen={true}
>
  <div class="flex flex-col gap-4 p-4 min-w-[300px]">
    <div class="text-sm text-[var(--text-secondary)] mb-2">
      {$_("dashboard.tpslManager.symbol")} <span class="text-[var(--text-primary)] font-bold"
        >{order?.symbol}</span
      >
    </div>
    ...
    <div class="flex flex-col gap-1">
      <label
        ...
        >{$_("dashboard.tpslManager.triggerPrice")}</label
      >
      <input
    ...
    <div class="flex flex-col gap-1">
      <label
        for="tpsl-amount"
        class="text-xs font-bold text-[var(--text-secondary)]"
        >{$_("dashboard.tpslManager.qty")}</label
      >
      <input
        id="tpsl-amount"
        name="tpslAmount"
    ...
    <div class="flex justify-end gap-2 mt-2">
      <button
        ...
        disabled={loading}
      >
        {$_("dashboard.tpslManager.cancel")}
      </button>
      <button
        ...
        disabled={loading}
      >
        {loading ? $_("dashboard.tpslManager.saving") : $_("dashboard.tpslManager.save")}
      </button>
    </div>
  </div>
</ModalFrame>
