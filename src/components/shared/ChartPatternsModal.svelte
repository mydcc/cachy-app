<script lang="ts">
  ...
  import { CHART_PATTERNS, type ChartPattern } from "../../services/chartPatterns";
  ...
  let selectedPatternId = $state(CHART_PATTERNS.length > 0 ? CHART_PATTERNS[0].id : null);
  ...
  function toggleFavorite(id: string) {
      ...
      localStorage.setItem("chart_pattern_favorites", JSON.stringify([...newFavorites]));
  }

  // Derived filtered list
  let filteredPatterns = $derived(CHART_PATTERNS.filter(p => {
      // Localization fallback for name if needed, but we usually have it in the object or i18n
      const localizedName = $_(`chartPatterns.${p.id}.name`) || p.name;
  ...
  const categories = $derived([
      "All",
      "Favorites",
      ...new Set(CHART_PATTERNS.map(p => p.type))
  ].sort((a, b) => {
      if (a === "All") return -1;
      if (b === "All") return 1;
  ...
  // Current Pattern
  let currentPattern = $derived(CHART_PATTERNS.find(p => p.id === selectedPatternId) || CHART_PATTERNS[0]);
  ...
  function getLocalizedText(patternId: string, key: string): string {
      const i18nKey = `chartPatterns.${patternId}.${key}` as any;
      const text = $_(i18nKey);
      if (text === i18nKey) {
          return ""; // Return empty if missing to hide section or show fallback
      }
      return text;
  }
...
{#if uiState.showChartPatternsModal}
  <ModalFrame
    isOpen={true}
    title={$_("chartPatterns.title") || "Chart Patterns Academy"}
    onclose={() => uiState.toggleChartPatternsModal(false)}
                ...
                {#each filteredPatterns as pattern}
                    ...
                    >
                        <div class="flex items-center gap-2 truncate">
                             ...
                             {/if}
                             <span class="truncate">{$_(`chartPatterns.${pattern.id}.name`) || pattern.name}</span>
                        </div>
                    </button>
                {/each}
            ...
            {#if currentPattern}
                ...
                <div>
                    <div class="flex justify-between items-start">
                        <div class="flex items-center gap-3">
                            <h2 class="text-2xl font-bold text-[var(--accent-color)]">
                                {$_(`chartPatterns.${currentPattern.id}.name`) || currentPattern.name}
                            </h2>
                            <button
                ...
                <div class="flex flex-col lg:flex-row gap-6 h-full">
                    ...
                    <div class="w-full lg:w-2/3 flex flex-col gap-6">
                        ...
                        <!-- Characteristics -->
                        <div class="bg-[var(--bg-tertiary)] p-4 rounded-xl border border-[var(--border-color)]">
                            <h3 class="text-sm font-bold uppercase text-[var(--text-secondary)] mb-2">Characteristics</h3>
                            <div class="prose dark:prose-invert text-sm max-w-none">
                                {@html renderMarkdown(getLocalizedText(currentPattern.id, 'characteristics'))}
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="w-full lg:w-1/3 flex flex-col gap-6 h-full">
                         <!-- Trading -->
                         <div class="bg-[var(--bg-tertiary)] p-4 rounded-xl border border-[var(--border-color)]">
                            <h3 class="text-sm font-bold uppercase text-[var(--text-secondary)] mb-2">Trading Strategy</h3>
                            <div class="prose dark:prose-invert text-sm max-w-none">
                                {@html renderMarkdown(getLocalizedText(currentPattern.id, 'trading'))}
                            </div>
                        </div>

                        <!-- Interpretation / Advanced -->
                        <div class="bg-[var(--bg-tertiary)] p-4 rounded-xl border border-[var(--border-color)] flex-grow">
                            <h3 class="text-sm font-bold uppercase text-[var(--text-secondary)] mb-2">Interpretation & Context</h3>
                            <div class="prose dark:prose-invert text-sm max-w-none">
