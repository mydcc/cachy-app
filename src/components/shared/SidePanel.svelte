<script lang="ts">
    ...
    function handleKeydown(e: KeyboardEvent) {
        if (e.key === 'Enter' && !e.shiftKey) { // Allow shift+enter for new lines if textarea (future)
             e.preventDefault();
            handleSend();
        }
    }
            ...
            >
                    ...
                    <div class="flex items-center gap-2">
                        {#if $settingsStore.sidePanelMode === 'ai'}
                             ...
                             <button
                                ...
                                on:click={() => confirm('Clear chat history?') && aiStore.clearHistory()}
                                title="Clear History"
                            >
                ...
                <div class="p-3 border-t border-[var(--border-color)] bg-[var(--bg-secondary)]">
                    {#if errorMessage || $aiStore.error}
                        <div class="text-xs text-[var(--danger-color)] mb-2 animate-pulse">{errorMessage || $aiStore.error}</div>
                    {/if}
                    <div class="relative">
                        <input
                            bind:this={inputEl}
