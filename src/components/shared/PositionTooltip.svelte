...
<div class="bg-[var(--bg-tertiary)] border border-[var(--border-color)] shadow-xl rounded-lg p-3 text-xs text-[var(--text-primary)] w-[320px] max-w-[90vw] pointer-events-none z-[9999]">
    <!-- Header -->
    <div class="flex justify-between items-center mb-2 border-b border-[var(--border-color)] pb-1">
        ...
        <span class="font-mono text-[var(--text-secondary)] uppercase">{position.marginMode}</span>
    </div>
    ...
    <div class="grid grid-cols-2 gap-x-4 gap-y-1">
        ...
        <div class="flex justify-between">
         ...
         <div class="flex justify-between">
            ...
            <span>{formatDynamicDecimal(new Decimal(position.size).mul(position.markPrice || position.entryPrice))}</span>
        </div>
