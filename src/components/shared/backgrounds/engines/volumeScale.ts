...
/** Tunables for {@link PriceRangeTracker}. */
export interface PriceRangeOptions {
	/** How many recent trade prices define the window. */
	sampleSize?: number;
	/** Lower percentile (0..1) mapped to a normalised 0. */
	lowPercentile?: number;
	/** Upper percentile (0..1) mapped to a normalised 1. */
	highPercentile?: number;
	/**
	 * Minimum window width, as a fraction of the window's own midpoint. A market
	 * that prints the same price for a minute would otherwise collapse the
	 * window to zero width and turn one tick of noise into a full-scale swing.
	 */
	minRelSpread?: number;
	/** Smoothing factor per update for the anchors (0..1, higher = snappier). */
	adaptRate?: number;
...
const PRICE_DEFAULTS = {
	...
	lowPercentile: 0.02,
	highPercentile: 0.98,
	// 0.1% of price — the same order as BlockEngine's ±0.5% first-trade margin.
	minRelSpread: 0.001,
	// Matches BlockEngine's own bound smoothing, so both price axes in the app
	// chase a moving market at the same rate.
	adaptRate: 0.05
} as const;

/**
 * Maps trade prices onto a normalised `0..1` position within an adaptive window
 * of recent prices — the price counterpart of {@link VolumeNormalizer}.
 *
 * Deliberately LINEAR, unlike the volume normaliser's log curve: notionals span
 * orders of magnitude within a single symbol, prices do not. `log10(90000)` and
 * `log10(90500)` differ by 0.0024, so a log transform would flatten an entire
 * intraday range onto the middle of the scale.
 *
 * Uses percentile anchors rather than the raw min/max that `BlockEngine`
 * computes inline, so one bad print cannot pin the scale until it ages out of
 * the window.
 */
export class PriceRangeTracker {
	private readonly sampleSize: number;
	private readonly lowPercentile: number;
	private readonly highPercentile: number;
	private readonly minRelSpread: number;
	private readonly adaptRate: number;
	...
	constructor(options: PriceRangeOptions = {}) {
		this.sampleSize = Math.max(8, Math.floor(options.sampleSize ?? PRICE_DEFAULTS.sampleSize));
		this.lowPercentile = clamp01(options.lowPercentile ?? PRICE_DEFAULTS.lowPercentile);
		this.highPercentile = clamp01(options.highPercentile ?? PRICE_DEFAULTS.highPercentile);
		this.minRelSpread = Math.max(1e-6, options.minRelSpread ?? PRICE_DEFAULTS.minRelSpread);
		this.adaptRate = clamp01(options.adaptRate ?? PRICE_DEFAULTS.adaptRate);
		this.samples = new Float64Array(this.sampleSize);
		this.sorted = new Float64Array(this.sampleSize);
	}

	/**
	 * Feeds a price and returns where it sits in the current window, `0` being
	 * the bottom of the recent range and `1` the top. Both ends are soft: a
	 * price beyond the window clamps rather than running off the scale, and the
	 * window moves toward it over the next few trades.
	 *
	 * The very first price returns `0.5` — with nothing to compare against, the
	 * only honest answer is "mid-range", not "at the bottom".
	 */
	public push(price: number): number {
		if (!Number.isFinite(price) || price <= 0) return 0.5;

		this.samples[this.writeIdx] = price;
		this.writeIdx = (this.writeIdx + 1) % this.sampleSize;
		if (this.sampleCount < this.sampleSize) this.sampleCount++;

		this.updateAnchors(price);
		return this.normalizePrice(price);
	}

	/** Positions a price against the current window without feeding the sample. */
	public normalize(price: number): number {
		if (!Number.isFinite(price) || price <= 0) return 0.5;
		return this.normalizePrice(price);
	}

	/** Current window in raw price terms — for legends and axis labels. */
	public getRange(): { low: number; high: number } {
		if (!this.hasAnchors) return { low: 0, high: 0 };
		return { low: this.lowAnchor, high: this.highAnchor };
	}
	...
	private normalizePrice(price: number): number {
		...
		const spread = this.highAnchor - this.lowAnchor;
		if (spread <= 0) return 0.5;
		return clamp01((price - this.lowAnchor) / spread);
	}

	private updateAnchors(latest: number): void {
		const count = this.sampleCount;
		for (let i = 0; i < count; i++) this.sorted[i] = this.samples[i];
		const view = this.sorted.subarray(0, count);
		view.sort();
		...
		// Keep the window from collapsing onto a single price.
		const mid = (low + high) * 0.5 || latest;
		const minSpread = Math.abs(mid) * this.minRelSpread;
		if (high - low < minSpread) {
			low = mid - minSpread * 0.5;
			high = mid + minSpread * 0.5;
		}
