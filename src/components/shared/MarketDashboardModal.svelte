    ...
    >
        <div class="space-y-3 sm:space-y-6">
            ...
            <!-- Market Internals: three cards from sm up; below sm ONE
                 compact strip -- stacked full-width panels ate ~300px of
                 phone viewport before any symbol row, while heat / breadth /
                 top are a single glance. Snippets keep one source per KPI;
                 only the chrome differs between the two variants. -->
            {#snippet kpiHeatInner()}
                <div
            ...
            <!-- From sm up: the familiar three-panel row -->
            <div class="hidden sm:grid grid-cols-3 gap-2 sm:gap-3 md:gap-4">
                <div
            ...
            <!-- Below sm: one glanceable strip instead of three panels -->
            <div
                class="sm:hidden bg-[var(--bg-tertiary)] rounded-lg border border-[var(--border-color)] flex divide-x divide-[var(--border-color)] overflow-hidden"
            >
                <div
                    class="flex-1 min-w-0 px-2 py-1.5 flex flex-col items-center justify-center gap-0.5 text-center"
                >
                    <span
                        class="text-[9px] uppercase tracking-wide text-[var(--text-secondary)] truncate max-w-full"
                        >{$_("app.marketDashboard.marketHeat")}</span
                    >
                    {#if avgRsi === null}
                        <span
                            class="text-[13px] font-semibold text-[var(--text-secondary)]"
                            >—</span
                        >
                    {:else}
                        <span
                            class="text-[13px] font-semibold truncate max-w-full {avgRsi >= 70
                                ? 'text-[var(--danger-color)]'
                                : avgRsi <= 30
                                  ? 'text-[var(--success-color)]'
                                  : 'text-[var(--text-primary)]'}"
                        >
                            {avgRsi.toFixed(0)} · {heatZone}
                        </span>
                    {/if}
                </div>
                ...
                >
                    <span
                        class="text-[9px] uppercase tracking-wide text-[var(--text-secondary)] truncate max-w-full"
                        >{$_("app.marketDashboard.marketBreadth")}</span
                    >
                    {#if breadth === null}
                        <span
                            class="text-[13px] font-semibold text-[var(--text-secondary)]"
                    ...
                    {:else}
                        <span
                            class="text-[13px] font-semibold truncate max-w-full {breadth.percent > 50
                                ? 'text-[var(--success-color)]'
                                : 'text-[var(--danger-color)]'}"
                        >
                            {breadth.bullish}/{breadth.measured}
                        </span>
                    {/if}
                </div>
                ...
                >
                    <span
                        class="text-[9px] uppercase tracking-wide text-[var(--text-secondary)] truncate max-w-full"
                        >{$_("app.marketDashboard.topOpportunity")}</span
                    >
                    {#if topPick}
                        {@const topSignal = signalOf(topPick.analysis)}
                        <span
                            class="text-[13px] font-semibold truncate max-w-full"
                            style="color: {topSignal.color}"
                        >
                            {topPick.symbol.replace(/USDT(\.P|P)?$/, "")} ·
                            {topSignal.label}
                        </span>
                    {:else}
                        <span
                            class="text-[13px] font-semibold text-[var(--text-secondary)] truncate max-w-full"
                            >{$_("app.marketDashboard.scanning")}</span
                        >
                    {/if}
                </div>
            </div>
            ...
            >
                ...
                <!-- Table Body & Mobile Cards Container. divide-y owns the
                     row separators (one reliable line between children) --
                     per-row borders were too faint and partly dead v3
                     syntax. -->
                <div
                ...
                >
                    {#each rows as row (row.symbol)}
                        ...
                        <div
                            class="hidden md:grid grid-cols-12 gap-2 p-3 hover:bg-[var(--bg-primary)] transition-colors items-center text-sm group cursor-pointer row-selectable {row.analysed
                                ? ''
                                : 'opacity-60'}"
                        ...
                        <!-- MOBILE TICKER ROW (< md: 768px). One visual
                             line: symbol, compact trend pills, price+change,
                             colored score. Signal label, RSI, funding and
                             volume live in the badge's tap detail -- a phone
                             row is a ticker line, not a card. Selection
                             semantics unchanged: tap loads the symbol into
                             the calculator; closes after an analysed pick. -->
                        <div
                            class="md:hidden px-3 py-2 hover:bg-[var(--bg-primary)] transition-colors flex flex-wrap items-center gap-x-2 gap-y-1 cursor-pointer row-selectable {row.analysed
                                ? ''
                                : 'opacity-60'}"
                            class:row-selected={activeSymbol ===
                                row.symbol.toUpperCase()}
                        ...
                        >
                            ...
                            <div class="flex items-center gap-0.5 shrink-0" aria-hidden="true">
                                <div class="w-1.5 h-3 rounded-xs {trendCellClass(trends?.['15m'])}"></div>
                                <div class="w-1.5 h-3 rounded-xs {trendCellClass(trends?.['1h'])}"></div>
                                <div class="w-2 h-3.5 rounded-xs {trendCellClass(trends?.['4h'])} ring-1 ring-[var(--bg-tertiary)]"></div>
                                <div class="w-1.5 h-3 rounded-xs {trendCellClass(trends?.['1d'])}"></div>
                            </div>
                            ...
                            <div class="ml-auto flex items-center gap-2.5 shrink-0">
                                ...
                                <!-- Score badge: the border+number color carries
                                     the signal word; tapping expands label, RSI,
                                     funding and reasoning inline. Touch has no
                                     hover -- this is its only affordance. -->
                                {#if row.analysed}
                                    <button
                                        type="button"
                                        class="w-7 h-7 shrink-0 rounded-full flex items-center justify-center text-[11px] font-bold border bg-[var(--bg-secondary)] cursor-pointer"
                                        style="border-color: {signal.color}; color: {signal.color}"
                                ...
                                {:else}
                                    <span class="text-[10px] text-[var(--text-secondary)] italic">
                                        {$_("app.marketDashboard.noData")}
                                    </span>
                                {/if}
                            </div>

                            <!-- Expanded tap detail: everything that left the
                                 ticker line, plus the score's reasoning. -->
                            {#if expandedSymbol === row.symbol && row.analysed}
                                <div
                                    class="w-full pt-2 mt-1 border-t border-[var(--border-color)]/40 text-[10px] leading-relaxed text-[var(--text-secondary)]"
                                >
                                    <div class="flex flex-wrap gap-x-3 gap-y-0.5 mb-1">
                                        <span class="font-sans font-bold" style="color: {signal.color}">
                                            {signal.label}
                                        </span>
                                        {#if rsiNum !== null}
                                        ...
                                        {#if volumeOf(row.symbol)}
                                            <span>{$_("app.marketDashboard.volume24h")} {volumeOf(row.symbol)}</span>
                                        {/if}
                                    </div>
