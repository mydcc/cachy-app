                ...
                <div class="bg-[var(--bg-primary)] rounded-lg p-2 border border-[var(--border-primary)]">
                    <div class="flex justify-between items-center mb-1">
                        ...
                        <div class="text-right">
                             <span class="text-xs font-bold px-1.5 py-0.5 rounded uppercase block w-fit ml-auto"
                    ...
                    <div class="flex justify-between text-xs mb-1">
                         <span class="text-[var(--text-secondary)]">{new Date(order.time).toLocaleTimeString()}</span>
                         <span class="text-[var(--text-primary)]" class:text-[var(--success-color)]={order.status === 'FILLED'} class:text-[var(--text-secondary)]={order.status !== 'FILLED'}>{order.status}</span>
                    </div>

                    <div class="flex justify-between text-xs">
                        <span class="text-[var(--text-secondary)]">PnL:</span>
                        <span class:text-[var(--success-color)]={order.realizedPnL > 0} class:text-[var(--danger-color)]={order.realizedPnL < 0}>
