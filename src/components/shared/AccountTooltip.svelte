<script lang="ts">
    ...
    $: equity = (account.available || 0) + (account.margin || 0) + (account.frozen || 0) + (account.totalUnrealizedPnL || 0);
...
<div class="bg-[var(--bg-tertiary)] border border-[var(--border-color)] shadow-xl rounded-lg p-3 text-xs text-[var(--text-primary)] w-[260px] pointer-events-auto z-50">
    <!-- Header: Equity -->
    <div class="flex flex-col mb-2 border-b border-[var(--border-color)] pb-2">
        <div class="flex justify-between items-center mb-1">
            ...
            <span class="font-bold text-sm">{formatDynamicDecimal(equity, 2)} {account.marginCoin || 'USDT'}</span>
        </div>

        <!-- Health Bar -->
        <div class="w-full h-1.5 bg-[var(--bg-primary)] rounded-full overflow-hidden mt-1 relative group">
            <div class="h-full transition-all duration-300 rounded-full"
                 style="width: {barWidth}%; background-color: {healthColor};">
            </div>
        </div>
        <div class="flex justify-between mt-1 text-[10px] text-[var(--text-secondary)]">
    ...
    <div class="grid grid-cols-1 gap-1">
        <div class="flex justify-between">
            ...
            <span>{formatDynamicDecimal((account.available || 0) + (account.margin || 0) + (account.frozen || 0), 2)}</span>
        </div>

        <div class="pl-2 border-l-2 border-[var(--border-primary)] ml-1 flex flex-col gap-0.5 my-1">
             <div class="flex justify-between">
                <span class="text-[var(--text-secondary)] text-[10px]">Transferable:</span>
        ...
        <div class="flex justify-between pt-1 border-t border-[var(--border-color)]">
