...
<script lang="ts">
    ...
    let processedTrades = $derived(
        journalState.entries.filter((trade) => {
            ...
            if (filterDateStart)
                matchesDate =
                    matchesDate && tradeDate >= new Date(filterDateStart);
            if (filterDateEnd) {
    ...
    function confirmDeleteTrade(tradeId: string) {
    ...
    async function handleScreenshotUpload(id: string, file: File) {
        try {
            ...
            {#if settingsState.isPro}
                {#if uiState.syncProgress}
                    ...
                    >
                        ...
                        >
                            {uiState.syncProgress.current}/{uiState.syncProgress
                                .total}
                        </span>
                        <div
