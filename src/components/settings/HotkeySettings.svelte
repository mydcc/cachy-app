<script lang="ts">
    ...
    import { HOTKEY_ACTIONS, type HotkeyAction, normalizeKeyCombo } from '../../services/hotkeyService';
    ...
    function handleKeyDown(event: KeyboardEvent) {
        ...
        if (existingAction) {
            ...
            if (!confirm(`"${newCombo}" is used by "${existingAction.label}". Overwrite?`)) {
                return;
            }
...
<div class="flex flex-col gap-4 h-full">
    <div class="flex justify-between items-center mb-2">
        <p class="text-xs text-[var(--text-secondary)]">
            Click on a key combination to record a new hotkey.
            Use <span class="text-[var(--accent-color)]">Alt</span> or <span class="text-[var(--accent-color)]">Ctrl</span> modifiers to avoid conflicts with typing.
        </p>
        <button class="text-xs text-[var(--danger-color)] hover:underline" on:click={resetToDefaults}>
            Reset All
        </button>
    </div>
                    ...
                    {#each groupedActions[category] as action}
                        <div class="flex justify-between items-center p-2 rounded bg-[var(--bg-tertiary)] border border-[var(--border-color)]">
                            ...
                            <button
                                ...
                                on:click={(e) => { e.stopPropagation(); startEditing(action.id); }}
                            >
                                {editingId === action.id ? 'Press Key...' : getDisplayKey(action)}
                            </button>
                        </div>
                    {/each}
                </div>
    ...
    {#if conflictWarning}
        <div class="fixed bottom-4 right-4 bg-[var(--warning-color)] text-black px-4 py-2 rounded shadow-lg text-sm font-bold animate-bounce">
            {conflictWarning}
        </div>
    {/if}
</div>
