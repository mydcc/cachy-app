    ...
    static scan(
        ...
        for (let i = lowPivots.length - 1; i > 0; i--) {
            ...
            for (let j = i - 1; j >= 0; j--) {
                ...
                const priceLow1 = Math.min(...priceLows.slice(Math.max(0, p1.index - searchWindow), Math.min(priceLows.length, p1.index + searchWindow + 1)));
                const priceLow2 = Math.min(...priceLows.slice(Math.max(0, p2.index - searchWindow), Math.min(priceLows.length, p2.index + searchWindow + 1)));
        ...
        for (let i = highPivots.length - 1; i > 0; i--) {
            ...
            for (let j = i - 1; j >= 0; j--) {
                ...
                const searchWindow = 2;
                const priceHigh1 = Math.max(...priceHighs.slice(Math.max(0, p1.index - searchWindow), Math.min(priceHighs.length, p1.index + searchWindow + 1)));
                const priceHigh2 = Math.max(...priceHighs.slice(Math.max(0, p2.index - searchWindow), Math.min(priceHighs.length, p2.index + searchWindow + 1)));
