...
export function redactString(input: string): string {
    ...
    out = out.replace(
        /\b([\w-]*(?:passw(?:or)?d|passphrase|secret|token|api[-_]?key|signature|authorization|bearer)[\w-]*)\s*[=:]\s*([^\s&,;"'}]+)/gi,
        (_match, key: string) => `${key}=${REDACTED}`,
    );

    // "key": "value" in embedded JSON
    out = out.replace(
        /(["'])([\w-]*(?:passw(?:or)?d|passphrase|secret|token|api[-_]?key|signature|authorization|bearer)[\w-]*)\1(\s*:\s*)(["'])(?:[^"'\\]|\\.)*\4/gi,
