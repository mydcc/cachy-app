...
import type { TechnicalsData, ConfluenceData, DivergenceItem } from "../services/technicalsTypes";
...
export class ConfluenceAnalyzer {
    ...
    static analyze(data: Partial<TechnicalsData>): ConfluenceData {
        ...
        const adjust = (amount: number, reason: string) => {
            score += amount;
            if (amount !== 0) contributing.push(`${amount > 0 ? '+' : ''}${amount} ${reason}`);
        };
        ...
        if (data.oscillators) {
            data.oscillators.forEach(osc => {
                const weight = 5;
                if (osc.action === "Buy" || osc.action === "Strong Buy") adjust(weight, `${osc.name} Bullish`);
                if (osc.action === "Sell" || osc.action === "Strong Sell") adjust(-weight, `${osc.name} Bearish`);
            });
        }
        ...
        if (data.advanced?.ichimoku) {
            ...
            else if (ichi.action === "Strong Sell") adjust(-20, "Ichimoku Strong Bear");
        }
        ...
        if (data.divergences) {
            data.divergences.forEach(div => {
                ...
                if (div.side === "Bullish") adjust(weight, `${div.type} Bull Div (${div.indicator})`);
