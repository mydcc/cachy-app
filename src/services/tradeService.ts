...
class TradeService {
    ...
    public async signedRequest<T>(
        ...
        if (!response.ok || (data.code !== undefined && String(data.code) !== "0")) {
            ...
            throw new BitunixApiError((data.code as string | number) || response.status || -1, "apiErrors.generic", rawMsg);
        }
    ...
    public async fetchTpSlOrders(view: "pending" | "history" = "pending"): Promise<TpSlOrder[]> {
        ...
        if (provider === "bitunix") {
                  ...
                  await Promise.all(
                      batch.map(async (sym) => {
                          try {
                              ...
                              const dataObj = data as { rows?: TpSlOrder[] };
                              const res = (Array.isArray(data) ? data : dataObj.rows || []) as TpSlOrder[];
                              results.push(...res);
                          } catch (e: unknown) {
             ...
             const dataObj = data as { rows?: TpSlOrder[] };
             const list = (Array.isArray(data) ? data : dataObj.rows || []) as TpSlOrder[];
