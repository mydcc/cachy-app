...
export class TradeExecutionService {
    ...
    async placeOrder(params: PlaceOrderParams): Promise<OrderResult> {
        ...
        if (params.timeInForce) {
            body.effect = params.timeInForce.toUpperCase().replace('POSTONLY', 'POST_ONLY');
        }
     ...
     */
    private mapOrderStatus(bitunixStatus: string): "pending" | "filled" | "cancelled" {
    ...
    async cancelOrder(orderId: string): Promise<void> {
        ...
        await this.signedRequest('POST', '/api/v1/futures/trade/cancel_orders', body);
    ...
    async cancelOrders(symbol: string): Promise<number> {
        ...
        console.log('[TradeService] Cancelled', cancelledCount, 'orders for', symbol);
    ...
    async closePosition(params: PositionCloseParams): Promise<OrderResult> {
        ...
        const oppositeSide: OrderSide = params.positionSide === 'long' ? 'sell' : 'buy';
     ...
     * @throws Error if unauthorized or API call fails
     */
    async flashClosePosition(symbol: string, positionSide: PositionSide): Promise<OrderResult> {
        TradeExecutionGuard.ensureAuthorized();
     ...
     * @throws Error if unauthorized or API call fails
     */
    async getHistoryOrders(symbol?: string, limit: number = 100): Promise<OrderResult[]> {
        TradeExecutionGuard.ensureAuthorized();

        console.log('[TradeService] Get History Orders, symbol:', symbol || 'all', 'limit:', limit);
    ...
    async getHistoryTrades(symbol?: string, limit: number = 100): Promise<any[]> {
        ...
        console.log('[TradeService] Get History Trades, symbol:', symbol || 'all', 'limit:', limit);
        const params = `?${symbol?`symbol=${symbol}&`:''}limit=${limit}`;
