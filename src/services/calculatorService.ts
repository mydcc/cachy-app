...
export class CalculatorService {
    constructor(
        ...
        private updateTradeStore: (updater: (state: TradeState) => TradeState) => void
    ) { }
    ...
    public calculateAndDisplay(): void {
        try {
            ...
            const validationResult = this.getAndValidateInputs(currentTradeState, newResults);
            ...
            if (validationResult.status === CONSTANTS.STATUS_VALID && validationResult.data) {
    ...
    private performCalculation(currentTradeState: TradeState, values: TradeValues, newResults: ResultsState) {
        ...
        if (!baseMetrics || baseMetrics.positionSize.lte(0)) {
            ...
            if (currentTradeState.isPositionSizeLocked) {
                ...
                this.updateTradeStore(state => ({ ...state, isPositionSizeLocked: false }));
            }
            return;
        }
    ...
    private getAndValidateInputs(currentTradeState: TradeState, newResults: ResultsState): {
        status: string;
        message?: string;
        fields?: string[];
        data?: TradeValues;
        ...
        newResults.isAtrSlInvalid =
            values.useAtrSl && values.stopLossPrice.lte(0);
