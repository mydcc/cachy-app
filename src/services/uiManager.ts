...
export function updateVisualBar(
  ...
  const validTargetPrices = targets
    .map((t) => parseDecimal(t.price))
    .filter((p) => p.gt(0));
  const allPrices = [
    parseDecimal(values.entryPrice),
    parseDecimal(values.stopLossPrice),
    ...validTargetPrices,
  ];
  ...
  const slPos = parseDecimal(values.stopLossPrice)
    .minus(lowestPrice)
    .dividedBy(totalRange)
    .times(100);
  const entryPos = parseDecimal(values.entryPrice)
    .minus(lowestPrice)
    .dividedBy(totalRange)
    .times(100);

  if (values.tradeType === CONSTANTS.TRADE_TYPE_LONG) {
    visualBarContent.push({
      type: "loss-zone",
      style: { left: `${slPos}%`, width: `${entryPos.minus(slPos)}%` },
    });
    visualBarContent.push({
      type: "gain-zone",
      style: {
        left: `${entryPos}%`,
        width: `${new Decimal(100).minus(entryPos)}%`,
      },
    });
  } else {
    visualBarContent.push({
    ...
    visualBarContent.push({
      type: "loss-zone",
      style: { left: `${entryPos}%`, width: `${slPos.minus(entryPos)}%` },
    });
  }
