...
vi.mock('talib-web', () => {
    return {
        ...
        STOCH: vi.fn().mockResolvedValue({ slowK: Array(100).fill(50), slowD: Array(100).fill(50) }),
        ...
        MACD: vi.fn().mockResolvedValue({ MACD: Array(100).fill(1), MACDSignal: Array(100).fill(0.5), MACDHist: Array(100).fill(0.5) }),
...
describe('technicalsService', () => {
    ...
    it('should respect custom settings', async () => {
        ...
        const result = await technicalsService.calculateTechnicals(klines, settings);
