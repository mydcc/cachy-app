...
// Initialize CalculatorService with required dependencies
const calculatorService = new CalculatorService(
  (showGuidance = false) => app.clearResults(showGuidance),
  (locked: boolean) => app.togglePositionSizeLock(locked)
);
...
export const app = {
  ...
  calculateAndDisplay: () => {
    // Delegate to CalculatorService for robust error handling
    calculatorService.calculateAndDisplay(tradeStore);
  },
...
if (!baseMetrics || baseMetrics.positionSize.lte(0)) {
  ...
  if (currentTradeState.isPositionSizeLocked)
    app.togglePositionSizeLock(false);
  return;
}
