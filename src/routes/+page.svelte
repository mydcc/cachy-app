<script lang="ts">
    ...
    function handleKeydown(event: KeyboardEvent) {
        if (event && event.key && event.key.toLowerCase() === "escape") {
            ...
            if ($uiStore.showWhitepaperModal)
                uiStore.toggleWhitepaperModal(false);
            if ($uiStore.showChangelogModal)
                uiStore.toggleChangelogModal(false);
            if (get(modalManager).isOpen)
...
<div class="relative w-full max-w-4xl mx-auto">
    ...
    >
        ...
        >
            ...
            >
                <div
                    class="flex items-center flex-wrap justify-end gap-2 md:order-1"
                >
                    <select
                        id="preset-loader"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4">
            ...
            <TradeSetupInputs
                ...
                on:selectSymbolSuggestion={(e) =>
                    app.selectSymbolSuggestion(e.detail)}
        ...
        <section
            id="results"
            class="mt-6 grid grid-cols-1 md:grid-cols-2 gap-x-8"
        >
            <div>
                <SummaryResults
                    isPositionSizeLocked={$tradeStore.isPositionSizeLocked}
                ...
                {#each $resultsStore.calculatedTpDetails as tpDetail}
                    <div class="result-group !mt-0 md:!mt-6">
                        <h2 class="section-header">
                            ...
                            {tpDetail.index + 1} ({tpDetail.percentSold.toFixed(
                                0,
                            )}%)
                        </h2>
                        <div class="result-item">
                            <span class="result-label"
                                >{$_("dashboard.riskRewardRatio")}</span
                            ><span
                                class="result-value"
...
>
    ...
    >
    <button
        class="text-link {$settingsStore.isPro
