...
export const POST: RequestHandler = async ({ request, fetch }) => {
  try {
    ...
    if (source === "cryptopanic") {
      ...
      const plans: ("developer" | "growth" | "enterprise")[] = ["developer", "growth", "enterprise"];
      ...
      for (const p of plans) {
        ...
        try {
          ...
          return json({ error: `Upstream error (${p}): ${response.status}`, details: errorText }, { status: response.status });
        } catch (e: any) {
          lastError = e.message;
          continue;
        }
      }
      return json({ error: "Could not find valid CryptoPanic endpoint", lastError }, { status: 404 });
