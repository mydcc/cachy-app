...
const CACHY_PATH = "/api/sync/positions-history";
const BITUNIX_BASE_URL = "https://fapi.bitunix.com";
const BITUNIX_PATH = "/api/v1/futures/position/get_history_positions";
...
export const POST: RequestHandler = async ({ request, getClientAddress }) => {
  ...
  const check = checkPresignedRequest(request, {
    ...
    rebuilt: canonicalQueryString(buildPositionsHistoryQueryParams(result.data)),
  });
  if (!check.ok) {
  ...
  try {
    const positions = await fetchBitunixHistoryPositions(check.envelope);
