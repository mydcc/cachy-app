...
NEVER: Recommend chasing a move that is more than 1 ATR extended from the last clear structure.`;
      ...
      const baseRoleInstructions = [
        ...
        "   - A trade setup MUST have a minimum Risk/Reward (R:R) ratio of 1:2 to be considered valid.",
        "   - If the user's proposed setup has an R:R below 1:1.5, REJECT IT entirely. Tell the user it's a bad trade mathematically.",
        "   - NEVER try to 'fix' a bad user setup by suggesting unrealistic or hallucinated Take Profit targets just to make the R:R look good. If the technical structure doesn't offer a logical 1:2 R:R, the trade is invalid.",
