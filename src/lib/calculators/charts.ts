...
import { calculateJournalStats, calculatePerformanceStats, getTagData, getTimingData, getDisciplineData as getDisciplineStats } from './stats';
...
export function getPsychologyData(journal: JournalEntry[]) {
    // Streak Analysis
    const sorted = [...journal].filter(t => t.status === 'Won' || t.status === 'Lost').sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());
...
export function getMonteCarloData(journal: JournalEntry[], simulations: number = 100, horizon: number = 100) {
    const closedTrades = journal.filter(t => t.status === 'Won' || t.status === 'Lost');
