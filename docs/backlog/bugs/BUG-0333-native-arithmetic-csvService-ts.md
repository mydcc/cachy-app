...
# BUG-0333 — Native arithmetic usage found in src/services/csvService.ts
...
- **Location**: `src/services/csvService.ts:118`
...
Replace the identified usage of native arithmetic at `src/services/csvService.ts:118` with safe decimal arithmetic (e.g. `Decimal`, `rust_decimal`).

## Acceptance criteria

- [ ] The identified usage of native arithmetic at `src/services/csvService.ts:118` is replaced with safe decimal arithmetic
