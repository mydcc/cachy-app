...
Update the Bitunix validation schemas in `src/types/bitunixValidation.ts` to strictly require strings for financial fields (`mp`, `ip`, `price`, `amount`, `size`, `dealAmount`, etc.). Reject raw numeric formats or use a transformation that avoids f64 conversion. Do not change actual logic that performs math.

## Acceptance criteria

- [ ] A test reproduces the defect by validating an API payload with an unrepresentable f64 value that loses precision.
- [ ] The Bitunix validation schemas strictly enforce string types for financial fields.
- [ ] The test passes with the fix (API payload is either rejected or correctly preserved as a string).
