...
# BUG-0371 — Native arithmetic usage found in src/stores/ai.svelte.ts
...
- **Location**: `src/stores/ai.svelte.ts:768`
...
Replace the identified usage of native arithmetic at `src/stores/ai.svelte.ts:768` with safe decimal arithmetic (e.g. `Decimal`, `rust_decimal`).

## Acceptance criteria

- [ ] The identified usage of native arithmetic at `src/stores/ai.svelte.ts:768` is replaced with safe decimal arithmetic
