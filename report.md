...
1.  **Type Safety & Validation in Execution Paths (`src/services/tradeService.ts`)**:
    *   **Finding**: Critical order execution functions rely heavily on `any` types. Specifically, `cancelTpSlOrder` takes untyped parameters or a loose object and `signedRequest` uses `Record<string, any>`, bypassing the TypeScript compiler entirely.
...
2.  **Generic API Serialization Risk (`src/services/tradeService.ts`)**:
    *   **Finding**: The `signedRequest` method accepts `payload: Record<string, any>` (currently `Record<string, unknown>`) and serializes it using `serializePayload(payload: any...)`.
...
3.  **Potential WebSocket Resource Leaks (`src/services/bitunixWs.ts`)**:
    *   **Finding**: `src/services/bitunixWs.leak.test.ts` exists, highlighting a known risk area. While explicit leaks weren't deeply confirmed in this read-only pass, `syntheticSubs` and `pendingSubscriptions` manage complex state. `destroy()` method is clearing maps but `syntheticSubs` bounds need to be confirmed.
...
1.  **Missing i18n & Hardcoded Errors (`src/services/tradeService.ts`)**:
    *   **Finding**: The system maps `TRADE_ERRORS.POSITION_NOT_FOUND` to `"tradeErrors.positionNotFound"`, but throws literal strings like `throw new Error("tradeErrors.positionNotFound")` or `throw new Error(TRADE_ERRORS.FETCH_FAILED)` while `trade.fetchFailed` is defined in schema.
    *   **Risk**: The frontend i18n library (e.g., `svelte-i18n`) might fail to find keys because the correct schema key might be different, displaying a raw, broken string to the user.
...
*   **Action**: In `tradeService.ts`, replace any usage of `any` with `TpSlOrder` or stricter types. `serializePayload` should use `unknown` to enforce type checking.
...
*   **Action**: Audit `bitunixWs.ts`. Implement bounded eviction strategies (e.g., maximum queue sizes) for pending arrays if necessary, and guarantee that `syntheticSubs.clear()` is called unconditionally during `ws.close` or reconnection cycles.
...
*   **Action**: In `tradeService.ts`, align the `TRADE_ERRORS` map directly with the exact keys in `src/locales/schema.d.ts` (e.g., `POSITION_NOT_FOUND: "tradeErrors.positionNotFound"`, `FETCH_FAILED: "trade.fetchFailed"`).
*   **Action**: Replace literal string throws (e.g., `throw new Error("apiErrors.missingCredentials")`) with centralized constants where possible.

### Group 4: Strict Number Parsing (🟡 WARNING)

**Justification:** Prevents loss of precision and adheres to financial standards.
*   **Action**: In `csvService.ts`, replace `parseFloat(originalIdAsString)` with `new Decimal(originalIdAsString).toNumber()`.
...
*   **Financial Standards**: Use `Decimal` types for ID fallback parsing in `csvService.ts`.
