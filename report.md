...
### Group 1: Type Safety & Validation Hardening (CRITICAL)

**Justification:** Measurably improves stability by ensuring the execution engine never receives structurally invalid data and eliminating `any` bypasses.
*   **Action**: In `tradeService.ts`, verify and replace `any` in `serializePayload(payload: unknown)` and `signedRequest`.
*   **Action**: Across `omsService.ts` and `apiService.ts`, refactor catch blocks to use `unknown` and type-narrowing instead of `(e as any)`.
*   **Unit Test to Reproduce**: Create a test for `serializePayload` passing unexpected objects to ensure safe fallback.

### Group 2: Financial Precision Enforcement (CRITICAL)

**Justification:** Prevents financial loss due to floating-point inaccuracies by maintaining Decimal end-to-end.
*   **Action**: In `julesService.ts`, refactor properties to accept `Decimal` directly instead of calling `.toNumber()`.
*   **Unit Test to Reproduce**: Create a calculation test in `julesService.test.ts` where a small float discrepancy changes the outcome before the fix, and is resolved after.

### Group 3: Memory Leak & Resource Management (WARNING / REFACTOR)

**Justification:** Prevents platform crashes during long sessions and memory exhaustion.
*   **Action**: In `toastService.svelte.ts`, bind the `setTimeout` and ensure proper cleanup if dismissed. Implement bounded size for the toast array.
*   **Action**: Ensure `syntheticSubs.clear()` and other Maps/Sets are cleared deterministically in teardown methods across services.

### Group 4: i18n & Error Handling Stabilization (WARNING)

**Justification:** Improves UX and broken state resilience.
*   **Action**: Identify hardcoded strings in settings/indicator components and replace with i18n keys.
*   **Action**: Standardize API error catches to map to safe, localized error keys instead of leaking raw `message` or HTML.

### Execution Guidelines Adherence
*   **Defensive Programming**: We assume the worst-case for API responses, catching and mapping errors cleanly.
*   **No Regressions**: No structural payload changes, only strict TypeScript enforcement.
*   **Financial Standards**: We strictly adhere to Decimal usage and remove `.toNumber()` from critical financial code paths.
